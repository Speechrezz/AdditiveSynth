...
{
    for (auto& voice : synthVoiceArray)
        voice.renderNextBlock(outputAudio, prevSamplePos, numSamples);
}
...
void SynthProcessor::noteOn(const juce::MidiMessage& midiMessage)
{
    ...
    if (existingNoteIdx == -1)
    {
        ...
        if (emptyNoteIdx != -1)
        {
            synthVoiceArray[emptyNoteIdx].setQueuePos(newQueuePos);
            synthVoiceArray[emptyNoteIdx].startNote(midiNoteNumber, velocity, pitchWheelPosition);
            newQueuePos++;
        }
        ...
        else
        {
            ...
            synthVoiceArray[oldestIdx].setQueuePos(newQueuePos);
            synthVoiceArray[oldestIdx].startNote(midiNoteNumber, velocity, pitchWheelPosition);
    ...
    {
        synthVoiceArray[existingNoteIdx].startNote(midiNoteNumber, velocity, pitchWheelPosition);
        updateNoteQueue();
    }
}
